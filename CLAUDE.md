...
- `_extensions/diagrams/` — the extension (CSS, JS, Lua filter). Provides eight layouts: `.circle-flow` (nodes-on-a-ring), `.pie` (pie chart), `.process` (linear flow), `.pyramid` (stacked bands), `.matrix` (2×2 quadrants), `.hierarchy` (org chart / tree), `.venn` (2- or 3-set overlapping circles), and `.stacked-venn` (nested concentric circles). Each initializes from the same JS file (`initCircleFlow` / `initPie` / `initProcess` / `initPyramid` / `initMatrix` / `initHierarchy` / `initVenn` / `initStackedVenn`). `.hierarchy` is the only one that reads nested `.item` structure rather than a flat list.
